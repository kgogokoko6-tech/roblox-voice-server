...
// 1. روبلوكس يرسل الرمز هنا (HTTP GET/POST عادي)
...
// 2. روبلوكس يفحص هل اللاعب اتفعل
...
// 4. معالجة التفعيل عبر الـ Socket مباشرة (بدون POST وبدون HTTP 405)
io.on('connection', (socket) => {
    socket.on('verify-code-socket', (data) => {
        const code = String(data.code || '').trim();
        
        if (!code) {
            return socket.emit('verify-response', { success: false, message: 'برجاء كتابة الرمز!' });
        }
        ...
        if (activeCodes.has(code)) {
            ...
            console.log(`[Verified via Socket] Code ${code} verified!`);
            socket.emit('verify-response', { success: true, user: userData });
        } else {
            socket.emit('verify-response', { success: false, message: 'الكود غير صحيح أو انتهت صلاحيته!' });
        }
    });
